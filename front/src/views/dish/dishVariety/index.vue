...
<div class="search">
            ...
            <Form ref="searchForm" :model="searchForm" inline :label-width="0">
                <Form-item ref="searchForm" :model="searchForm" inline :label-width="0" style="display:flex;">
                    ...
                    <Form-item label="" prop="type">
                        <Select v-model="searchForm.type" placeholder="药品类型" clearable style="width: 150px">
                            <Option v-for="(item,index) in typeList" :key="index" :value="item.title">{{ item.title }}</Option>
                        </Select>
                    </Form-item>
                    <Form-item label="" prop="content">
                        <Input type="text" v-model="searchForm.content" placeholder="药品介绍" clearable style="width: 150px" />
                    </Form-item>
    ...
    <Modal v-model="buyModal" :title="buyTitle" :mask="false" footer-hide draggable width="1100">
        <Form :label-width="100" label-position="left">
            <Row :gutter="16">
                <Col span="24">
                <FormItem label="药品名称">
                    <Input v-model="buyObj.title" readonly style="width:100%" />
                </FormItem>
                </Col>
                <Col span="24">
                <FormItem label="加购数量">
                    <InputNumber v-model="buyNumber" min="0" max="5000000" placeholder="请输入加购数量..." style="width:100%"></InputNumber>
                </FormItem>
                </Col>
                <Col span="24">
                <Divider>
                    <Button @click="addOrderFx" :loading="buyLoading" type="success">提交加购</Button>
                </Divider>
                </Col>
            </Row>
        </Form>
...
export default {
                        ...
                        return h("div", [
                            h(
                                "Button", {
                                    ...
                                    on: {
                                        click: () => {
                                            this.buyObj = params.row;
                                            this.number = "";
                                            this.buyTitle = params.row.title;
                                            this.buyModal = true;
                                        }
                                    }
                                },
                                "已加购 " + params.row.buyNumber + "件"
                            ),
                            h(
                                "Button", {
                                    props: {
                                        type: params.row.collectFlag > 0 ? "warning" : "success",
                                        size: "small",
                                        icon: "ios-checkbox",
                                        ghost: true
                                    },
                                    ...
                                    on: {
                                        click: () => {
                                            if (params.row.collectFlag > 0) {
                                                this.cancelCollectFx(params.row);
                                            } else {
                                                this.addCollectFx(params.row);
                                            }
                                        }
                                    }
                                },
                                params.row.collectFlag > 0 ? "取消收藏" : "收藏"
                            ),
                            h(
                                "Button", {
                                    props: {
    ...
    methods: {
        ...
        getDataList() {
            this.loading = true;
            getDishVarietyList(this.searchForm).then(res => {
                this.loading = false;
                if (res.success) {
